...
public class PlayerScript : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        CheckPlayerPosition(); //스테이지 종료 감지 함수(추가)
    }
    public void SaveAndChangeScene()//데이터 저장 및 씬 전환(추가)
    {
        GameDataManager.instance.SaveData(gm.curritem, gm.price);
        SceneManager.LoadScene("StoreScene"); // 다음 씬으로 이동  
    }
    void CheckPlayerPosition() //스테이지 종료 감지 함수(추가)
    {
